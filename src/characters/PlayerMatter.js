...
export default class Player extends Phaser.Physics.Matter.Sprite {
    constructor(scene, x, y) { // Added texture and frame for clarity
        ...
        this.setCollidesWith([this.scene.foodItemCategory]);
        ...
        this.dangerZone = scene.matter.add.rectangle(x, y, 60, 60, {
            isSensor: true, // It detects collisions but doesn't resolve them physically
            isStatic: false, // It can move with the player
            label: 'dangerZone' // Give it a label for easy identification in collision events
        });
        // Attach the dangerZone body to the player's game object for easy access
        this.dangerZone.gameObject = this; // Or a reference back to the player if needed elsewhere
        ...
        this.scene.matter.world.on('collisionstart', this.handleMatterCollision, this);
    }
    ...
    handleMatterCollision(event, bodyA, bodyB) {
        ...
        const playerBody = this.body; // Reference to the player's actual physics body

        if (bodyA === playerBody || bodyB === playerBody) {
            // Determine which is the player body and which is the other body
            const otherBody = (bodyA === playerBody) ? bodyB : bodyA;
            const otherGameObject = otherBody.gameObject; // Get the Phaser GameObject from the Matter Body

            // Example: Collision with a static obstacle
            if (otherGameObject && otherGameObject.name === 'Obstacle' && !this.isDashing) {
                this.handleCollision(otherGameObject); // Call your existing collision logic
            }

            // Example: Player's dangerZone overlapping with a PowerUp
            if ((bodyA === this.dangerZone && otherGameObject.name === 'PowerUp') ||
                (bodyB === this.dangerZone && otherGameObject.name === 'PowerUp')) {
                const powerUp = (bodyA === this.dangerZone) ? otherGameObject : bodyA.gameObject;
                this.collectPowerUp(this.dangerZone, powerUp);
            }
        }
    }
    ...
    handleCollision(otherGameObject) { // Changed signature to accept the other GameObject
    ...
    update() {
        ...
        if (isMoving && !this.isStunned) {
            ...
            this.setFrame('orange_player_idle', crntDirectionIndex); // Assuming idle frames align with direction
            this.walkTimer.paused = true;
        }
